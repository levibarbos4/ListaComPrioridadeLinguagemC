...
int main() {
    setlocale(LC_ALL, "portuguese"); // seta local como português, desativar se for usar no linux
    ...
    while (1) {
        ...
        switch (escolha) {
            ...
            case 1:
                system("cls");
                inserir(&lista); // chama a função de inserir
                system("pause");
                puts("");
                break;
            case 2:
                system("cls");
                remover(&lista); // chamma a função para remover
                system("pause");
                puts("");
                break;
            case 3:
...
void inserir(listadecompras *lista) {
    ...
    if (aux == NULL) {  // insere caso a lista esteja vazia
        ...
        if (aux->proximo == NULL) {
            if (aux->categoria > novo->categoria) { // inserção pro prioridade
                novo->proximo = aux;
                aux->proximo = NULL;
                lista->inicio = novo;
                return;
...
// não estando limitada as 4 categorias
// final da lista não é necessário
// se for compilar lembre-se de alterar a codificação do arquivo para windows 1252
